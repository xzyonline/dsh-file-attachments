#!/bin/bash
# dsh-file-attachments 双击安装(macOS Finder 双击进入终端)
cd "$(dirname "$0")" || exit 1
...
npm install || { echo "[错误] 依赖安装失败,请检查网络后重试。"; read -r -p "按回车退出..."; exit 1; }

echo "[2/3] 部署插件(自动构建 + 链接 + 写补丁)..."
node scripts/install.mjs || { echo "[错误] 部署失败,请查看上方提示。"; read -r -p "按回车退出..."; exit 1; }
